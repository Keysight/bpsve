output "Agents" {
	value = {
		for name, module in module.Agents :
		name => {
			"admin_username" : module.Instance.admin_username
			"fqdn" : module.Eth0PublicIpAddress.fqdn
			"name" : module.Instance.name
			"private_ip_address" : module.Instance.private_ip_address
			"public_ip_address" : module.Eth0PublicIpAddress.ip_address
			"size" : module.Instance.size
		}
	}
}
